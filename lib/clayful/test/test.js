...
describe('Clayful', () => {
	...
	describe(`.[property]`, () => {
		...
		it(`should have Clayful's API end point as a baseUrl`, () => {
			expect(Clayful.baseUrl).to.be('https://api.clayful.io');
		});

		it(`should have an internal Promise constructor if Promise is available`, () => {
			expect(Clayful.Promise).to.be(Promise);
		});
	...
	describe(`.normalizeQueryValues()`, () => {
		...
		it(`should stringify & encode query string values`, () => {
			...
			const result = Clayful.normalizeQueryValues(query);
		...
		['GET', 'DELETE'].forEach(httpMethod => {
			...
			describe(httpMethod, () => {
				...
				it(`(...params, queryHeaders)`, done => {
					...
					expect(extracted.callback.Promise).to.be(Promise);
	...
	describe(`.callAPI()`, () => {
		...
		it(`should set a Promise property for the passed callback`, done => {

			const mockRequest = (detail, ClayfulError, callback) => {
				expect(callback).to.be.a('function');
				expect(callback.Promise).to.be(Promise);
				done();
			};
